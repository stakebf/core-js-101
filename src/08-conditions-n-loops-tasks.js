...
function isCreditCardNumber(ccn) {
  const res = ccn.toString().split('').reverse().map((num, indx) => {
    if (indx % 2 !== 0) {
      return (2 * num > 9) ? 2 * num - 9 : 2 * num;
    }
    return num;
  });
  return res.reduce((startValue, num) => +startValue + +num) % 10 === 0;
}
...
function getDigitalRoot(num) {
  let res = num.toString().split('').reduce((startValue, el) => +startValue + +el, 0);
  do {
    res = res.toString().split('').reduce((startValue, el) => +startValue + +el, 0);
  }
  while (res >= 10);
  return res;
}
